...
const setYouTubeAPIUrl = (song) => {
  const key = 'AIzaSyDpXXg4nm7kbAXhP7JjcnsXq4PTk1XuFew'
...
export function newApisCall({
  ...
  return function(dispatch, getState) {
    ...
    let itunesAPIUrl = setItunesAPIUrl(countryCode, totalNumSongs, genreNum)
...
const playNextVideo = (
  ...
  if (positionNext > songs.length) {
    ...
    //if start of list, play last video
...
export function handlePlayNextControl(e, nextBoolean) {
  return function(dispatch, getState) {
    ...
    let newActiveSongIdx = playNextVideo(
      players,
      songs,
      activeSongIdx,
      isMuted,
...
export function handlePlayPauseControl(e) {
  return function(dispatch, getState) {
    ...
    if (activeSongIdx !== -1) {
      ...
      //if no video played yet, play first video
      let isMuted = getState().isMuted
      let newActiveSongIdx = playNextVideo(
        players,
        songs,
